...
def write_model(model_path, input_base_path, tokenizer_path=None, safe_serialization=True, fix_eos_token_id=True):
    ...
    layer_bias_activation = olmo_config["layer_bias_activation"] if "layer_bias_activation" in olmo_config else []
    add_layer_bias = olmo_config["add_layer_bias"] if "add_layer_bias" in olmo_config else False
    add_embedding_transformation = olmo_config["add_embedding_transformation"] if "add_embedding_transformation" in olmo_config else False
    num_classes = olmo_config["num_classes"]
    ...
    for layer_ind in range(n_layers + 2):
        if f"transformer.layer_bias.{layer_ind}.weight" in loaded:
            state_dict[f"model.layer_bias.{layer_ind}.weight"] = loaded[f"transformer.layer_bias.{layer_ind}.weight"]

    if "transformer.embedding_transformation_0.weight" in loaded:
        state_dict["model.embedding_transformation_0.weight"] = loaded["transformer.embedding_transformation_0.weight"]

    if "transformer.embedding_transformation_1.weight" in loaded:
        state_dict["model.embedding_transformation_1.weight"] = loaded["transformer.embedding_transformation_1.weight"]
    ...
    config = OlmoCustomConfig(
        layer_bias_activation=layer_bias_activation,
        add_layer_bias=add_layer_bias,
        add_embedding_transformation=add_embedding_transformation,
    ...
    model = CustomOlmoForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float32, low_cpu_mem_usage=True)
