...
class CustomOLMo(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if self.config.block_group_size > 1:
            raise NotImplementedError("Block groups are not yet supported for custom models! Please set block_group_size = 1")
            block_groups = [
        ...
        # create class_bias for each layer and stores it in a modulelist
        # if config.add_layer_bias:
        if config.add_layer_bias:
            self.transformer.update(
                {
                    "layer_bias": nn.ModuleList(
                        [
                            nn.Embedding(config.num_classes, config.d_model, device=config.init_device)
        ...
        if config.add_embedding_transformation:
            assert config.num_classes == 2, "Only binary classification is supported"
            # we add a linear layer for each possible class
            self.transformer.update(
                {
                    "embedding_transformation_0": nn.Linear(
                        config.d_model,
                        config.d_model,
                        device=config.init_device,
                    )
            ...
            self.transformer.update(
                {
                    "embedding_transformation_1": nn.Linear(
                        config.d_model,
                        config.d_model,
                        device=config.init_device,
                    )
    ...
    def reset_parameters(self):
        ...
        # loop through layerbias and initialize for each
        if hasattr(self.transformer, "layer_bias"):
            for layer_bias in self.transformer.layer_bias:
                init_weights(self.config, layer_bias, type_of_module=ModuleType.emb)

        # TODO delete
        # embedding transformation
        if hasattr(self.transformer, "embedding_transformation_0"):
            init_weights(self.config, self.transformer.embedding_transformation_0, type_of_module=ModuleType.final_out)

        if hasattr(self.transformer, "embedding_transformation_1"):
            init_weights(self.config, self.transformer.embedding_transformation_1, type_of_module=ModuleType.final_out)
    ...
    def forward(
        ...
        :param attention_mask: A tensor of shape `(batch_size, seq_len)` that indicates
            ...
            library.
        :param class_labels: A tensor of shape '(batch_size, seq_len)' that stores which token belongs to which label class
            This is only used when add_class_bias is True or when add_embedding_bias is True
        ...
        # adds the embedding bias by checking if 0 is in layer_bias_activation
        if self.config.add_layer_bias:
            if 0 in self.config.layer_bias_activation:
                assert class_labels is not None
                # class_labels shape: (batch_size, seq_len)
                # layer_bias shape: (NUM_CLASSES, d_model)
                bias_for_each_token = self.transformer.layer_bias[0](class_labels)
                x = x + bias_for_each_token


        # TODO delete
        # adds the embedding_transformation
        if self.config.add_embedding_transformation:
            assert class_labels is not None

            new_x0 = self.transformer.embedding_transformation_0(x)
            new_x1 = self.transformer.embedding_transformation_1(x)

            x = torch.where(class_labels.unsqueeze(-1) == 0, new_x0, new_x1)
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                # adds the embedding bias if block_idx is in layer_bias_activation
                # we add 1 to block_idx to account for embedding layer
                if self.config.add_layer_bias:
                    if block_idx + 1 in self.config.layer_bias_activation:
                        assert class_labels is not None
                        # class_labels shape: (batch_size, seq_len)
                        # layer_bias shape: (NUM_CLASSES, d_model)
                        bias_for_each_token = self.transformer.layer_bias[block_idx + 1](class_labels)
                        x = x + bias_for_each_token
            ...
            # this should never be entered when block_group_size = 1
        ...
        # we add 1 to block_idx to account for embedding layer
        if self.config.add_layer_bias:
            if self.config.n_layers + 1 in self.config.layer_bias_activation:
                assert class_labels is not None
                # class_labels shape: (batch_size, seq_len)
                # layer_bias shape: (NUM_CLASSES, d_model)
                bias_for_each_token = self.transformer.layer_bias[self.config.n_layers + 1](class_labels)
                x = x + bias_for_each_token
    ...
    def get_fsdp_wrap_policy(self, wrap_strategy: Optional[FSDPWrapStrategy] = None):
        ...
        if hasattr(self.transformer, "class_bias"):
            size_based_module_to_wrap.add(self.transformer.class_bias)
        if hasattr(self.transformer, "embedding_bias"):
            size_based_module_to_wrap.add(self.transformer.embedding_bias)
        if hasattr(self.transformer, "embedding_transformation_0"):
            size_based_module_to_wrap.add(self.transformer.embedding_transformation_0)
        if hasattr(self.transformer, "embedding_transformation_1"):
            size_based_module_to_wrap.add(self.transformer.embedding_transformation_1)
