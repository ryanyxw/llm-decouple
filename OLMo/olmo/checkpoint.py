    ...
    def restore_checkpoint(
        ...
        *,
        ...
        with FSDP.state_dict_type(
            ...
            # PRIVATE_EDIT: we don't want to nullify our custom models (and instead initialize them from scratch)
            def is_custom(name: str) -> bool:
                return "layer_bias" in name

            with torch.no_grad():
                # fill everything with NaN, so we can check afterwards that every parameter has been restored
                # CUSTOM_EDIT - we don't fill with NaN as a temporary workaround for custom components
                ...
                # this loops over all parameter weights and loads them from model.pt
                for module_name, module in fsdp_model.named_modules():
                    if not isinstance(module, FSDP):
                        continue
                    for param in module.params:
            ...
            # import pdb
            # pdb.set_trace()
            # count_2s_optim_before = []
            # count_2s_param_before = []
            # for group in optim.param_groups:
            #     for param in group["params"]:
            #         optim_param = optim.state[param]
            #         if optim_param["exp_avg"].shape != param.shape:
            #             import pdb
            #             pdb.set_trace()
            #         if 2 in optim_param["exp_avg"].shape:
            #
            #             count_2s_optim_before.append(param)
            #
            #         if 2 in param.shape:
            #             count_2s_param_before.append(param)
            #
            # import pdb
            # pdb.set_trace()
            ...
            # pdb.set_trace()
            #
            # count_2s_optim_after = []
            # count_2s_param_after = []
            #
            # group_counter = 0
            # for group in optim.param_groups:
            #     for name, param in zip(group["param_names"], group["params"]):
            #         optim_param = optim.state[param]
            #         if "exp_avg" not in optim_param:
            #             import pdb
            #             pdb.set_trace()
            #             continue
            #         if optim_param["exp_avg"].shape != param.shape:
            ...
            #             pdb.set_trace()
            #         if 2 in optim_param["exp_avg"].shape:
            #             count_2s_optim_after.append(param)
            #
            #         if 2 in param.shape:
            #             count_2s_param_after.append(param)
            #     group_counter += 1
            #
            # import pdb
            # pdb.set_trace()
