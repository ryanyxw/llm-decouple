...
def get_args():
    ...
    group.add_argument(
        "--special_loss_mask",
        action="store_true",
        help="perform special masking that ONLY masks out the tokens in mask-before-token"
    )
    group.add_argument(
        "--percentage",
        type=float,
        default=1,
        help="the percentage of all sequences that will be masked"
    )
    group.add_argument(
        "--record_file_path",
        type=str,
        default="recorded_sequences",
        help="the file to store the recorded sequences and their masking decision"
    )
    group.add_argument(
        "--num-docs",
        default=None,
